...
import {Address, AddressWrapper} from './models/address.model.js';
...
async function fetchData() {
  try {
    ...
    const data: AddressWrapper = await response.json();
...
function parseData() {
  ...
  addresses.forEach(address => {
    ...
    const newRow = tableRowTemplate.cloneNode(true) as Element;
    ...
    address['gate_address'].forEach((glyph, i) => {
      ...
      if (i < 7) {
        updateText(
          newRow.querySelector(`.glyph-name-${i + 1}`),
          symbol?.['name'] ?? 'Unknown',
        );
    ...
    updateText(
      querySelector(`.info-name`, newRow),
      `${address['name']} # ${keyboardAddress}8`,
    );
    ...
    if (address['type'] === 'fan') {
      ...
      updateText(
        querySelector('.info-type', newRow).querySelectorAll('span')[1],
        'Fan',
      );
    } else {
      updateText(
    ...
    if (address['is_gate_online'] === '0') {
      ...
      updateText(
        querySelector('.status', newRow).querySelectorAll('span')[1],
        'Offline',
      );
    } else {
      updateText(
    ...
    if (address['ip_address']) {
      updateText(
        querySelector('.info-coord', newRow).querySelectorAll('span')[1],
        updateIp(address['ip_address']),
      );
    } else {
      updateText(
        querySelector('.info-coord', newRow).querySelectorAll('span')[0],
        '',
      );
      updateText(
        querySelector('.info-coord', newRow).querySelectorAll('span')[1],
    ...
    updateText(
      querySelector('.info-const', newRow).querySelectorAll('span')[1],
      Math.abs(hash(JSON.stringify(address))),
    );

    updateText(
      querySelector('.info-ref', newRow).querySelectorAll('span')[1],
...
function sortData(sortProperty: 'type' | 'glyph' | 'status' | 'name') {
...
function generatePlanetData() {
  let toReturn = [];
  let options = Object.keys(labelOptions) as (keyof typeof labelOptions)[];
  for (let i = 0; i < 3; i++) {
